...
webmap.factory('vacansiesList', ['$resource',
    function($resource){
        return function (txtSearch,lat,lng) {
            return $resource('https://api.hh.ru/vacancies?text=:search&clusters=true&isMap=true' +
                '&items_on_page=100&&enable_snippets=true&label=with_address' +
                '&bottom_left_lat=:latLeft&bottom_left_lng=:lngLeft'+
                '&top_right_lat=:latRight&top_right_lng=:lngRight',{}, {
                query: {method: 'GET',params:{search: txtSearch, latLeft:(lat-0.01), latRight:(lat+0.01), lngLeft:(lng-0.01), lngRight:(lng+0.01)},}
            });
         }
    }]);
